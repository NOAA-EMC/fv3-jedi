...
subroutine fv3jedi_register_field(fileobj, io_name, array, position, long_name, units, is_restart)
  ...
  if ( fileobj%is_readonly ) then ! For read
     ...
     if ( .not. is_registered ) then
        ...
        call register_field(fileobj, trim(xdim_name), 'double', (/ xdim_name /))
        call register_variable_attribute(fileobj, trim(xdim_name), 'long_name', trim(xdim_name), str_len=len(xdim_name))
     ...
     if ( .not. is_registered ) then
        ...
        call register_field(fileobj, trim(ydim_name), 'double', (/ ydim_name /))
        call register_variable_attribute(fileobj, trim(ydim_name), 'long_name', trim(ydim_name), str_len=len(ydim_name))
     ...
     if ( nz_field > 1 ) then
        ...
        if ( .not. is_registered) then
           ...
           call register_field(fileobj, trim(zdim_name), 'double', (/ zdim_name /))
           call register_variable_attribute(fileobj, trim(zdim_name), 'long_name', trim(zdim_name), str_len=len(zdim_name))
